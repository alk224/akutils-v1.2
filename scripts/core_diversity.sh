...
Generating phylogenetic tree plots based on input."
	...
	#		randcode1=`cat /dev/urandom |tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1` 2>/dev/null
			bash $scriptdir/phyloseq_tree.sh $CSSsort $mapfile $tree NULL phylum $outdir/Phyloseq_output/Trees/ &>/dev/null
			wait
	#			mv Phylum_tree_${randcode1}.pdf $outdir/Phyloseq_output/Trees/Phylum_tree.pdf 2>/dev/null

			for line in `cat $catlist`; do
	#		randcode1=`cat /dev/urandom |tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1` 2>/dev/null
			bash $scriptdir/phyloseq_tree.sh $CSSsort $mapfile $tree $line detail $outdir/Phyloseq_output/Trees/ &>/dev/null
			wait
...
Generating network plots for each supplied category."
			...
			for line in `cat $catlist`; do
		#	randcode1=`cat /dev/urandom |tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1` 2>/dev/null
			bash $scriptdir/phyloseq_network.sh $CSSsort $mapfile $line $outdir/Phyloseq_output/Networks/ &>/dev/null
			wait
